        ...
        b'w' => {
            if b.remaining() < 24 {
                return Err(ReplError::Protocol(format!(
                    "XLogData too short: {} bytes", b.remaining()
                )));
            }
            ...
            Ok(ReplicationCopyData::XLogData { wal_start, wal_end, server_time_micros, data })
        }
        b'k' => {
            if b.remaining() < 17 {
                return Err(ReplError::Protocol(format!(
                    "KeepAlive too short: {} bytes", b.remaining()
                )));
            }
            let wal_end = Lsn(b.get_i64() as u64);
            let server_time_micros = b.get_i64();
            let reply_requested = b.get_u8() != 0;
            Ok(ReplicationCopyData::KeepAlive { wal_end, server_time_micros, reply_requested })
        }
        _ => Err(ReplError::Protocol(format!("unknown CopyData kind: 0x{kind:02x}"))),
    }
}

pub fn encode_standby_status_update(applied: Lsn, client_time_micros: i64, reply_requested: bool) -> Vec<u8> {
...
pub fn parse_pgoutput_boundary(data: &Bytes) -> ReplResult<Option<PgOutputBoundary>> {
    ...
    fn take_i8(p: &mut &[u8]) -> ReplResult<i8> {
        if p.is_empty() { return Err(ReplError::Protocol("truncated i8".into())); }
        let v = p[0] as i8; *p = &p[1..]; Ok(v)
    }
    fn take_i32(p: &mut &[u8]) -> ReplResult<i32> {
        if p.len() < 4 { return Err(ReplError::Protocol("truncated i32".into())); }
        let (h, t) = p.split_at(4); *p = t;
        Ok(i32::from_be_bytes(h.try_into().unwrap()))
    }
    fn take_i64(p: &mut &[u8]) -> ReplResult<i64> {
        if p.len() < 8 { return Err(ReplError::Protocol("truncated i64".into())); }
        let (h, t) = p.split_at(8); *p = t;
    ...
    match tag {
        b'B' => {
            ...
            Ok(Some(PgOutputBoundary::Begin { final_lsn, commit_time, xid }))
        }
        b'C' => {
            ...
            let commit_time     = take_i64(&mut p)?;
            Ok(Some(PgOutputBoundary::Commit { lsn, end_lsn, commit_time }))
        }
        _ => Ok(None),
    }
}
...
pub enum PgOutputBoundary {
    Begin  { final_lsn: Lsn, commit_time: i64, xid: u32 },
